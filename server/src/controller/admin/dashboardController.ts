...
export class DashboardController implements IDashboardController {
    ...
    addUser = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const user = await this._dashboardSerice.addUser(firstName, lastName, phone, password)
            res.status(HttpStatus.OK).json({ user, success: true, message: "New user created successfully" })
        } catch (error) {
            const err = error as Error
            res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ success: false, message: err?.message || "Internal server error" });
        }
